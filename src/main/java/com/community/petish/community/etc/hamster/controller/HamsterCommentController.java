...
@RequestMapping("/etc/hamster/comments")
@RestController
public class HamsterCommentController {
	
	@Autowired
	private HamsterCommentService service;
	...
	public String getCommetListJSON(CommentCriteria cri, Model model) {
		...
		//게시글 내 댓글 전체
		List<HamsterCommentResponseDTO> list = service.getCommentList(cri);
		//댓글 저장할 dto
		List<HamsterCommentResponseDTO> commentDTOList = new ArrayList<HamsterCommentResponseDTO>();
		...
		for(int i=0; i<list.size(); i++) {
			//댓글 1개
			HamsterCommentResponseDTO commentDTO = list.get(i);			
			commentCount = service.getCommentCnt(cri.getPost_id());
			
			HamsterCommentResponseDTO newDTO = 
					new HamsterCommentResponseDTO(commentDTO.getId(), commentDTO.getContent(), 
	...
	public String getComment(@PathVariable("commentID") Long commentID) {
		...
		HamsterCommentResponseDTO dto = service.getComment(commentID);
	...
	public Map<String, Object> insertComment(HamsterCommentRequestDTO dto) {
	...
	public ResponseEntity<String> modifyComment(@RequestBody HamsterCommentRequestDTO dto, @PathVariable("commentID") Long commentID) {
		dto.setId(commentID);
